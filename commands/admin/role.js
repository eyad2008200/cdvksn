...
module.exports = {
        name: "رول",
        description: `تضيف او تشيل رتب من شخص.`,
        run: async (client, message, args) => {
              ...
              if (!args[0]) return message.reply({ content: `:rolling_eyes: **Please mention member or id**`, ephemeral: true }).catch((err) => {
                ...
                if (message.member.roles.highest.position < member.roles.highest.position)
                        return message.reply({
                                content:
                                        `:rolling_eyes: **You can't add role ${member.user.username} have higher role than you**`
                                , ephemeral: true
                        }).catch((err) => {
                                console.log(`i couldn't reply to the message: ` + err.message)
                        })
...
.setColor(message.guild.me.displayHexColor)        
   ...
   let embed = new MessageEmbed()
  .setDescription(`:white_check_mark:هاك ${member.user.username}, - **${role.name}**`)
.setColor(message.guild.me.displayHexColor)      
          message.reply({embeds: [embed]}).catch((err) => {
     console.log(`i couldn't reply to the message: ` + err.message) 
            })
