...
module.exports = {
        name: "مسح",
        description: `يمسح مسح واحد عراق`,
        ...
        const guilds = message.guild.me.permissions.has("MANAGE_MESSAGES");
                ...
                if (!permission)
                        return message.reply(
                                { content: ":x: **صلاحية ماعندك**", ephemeral: true }
         ).catch((err) => {
                                console.log(`i couldn't reply to the message: ` + err.message)
          })
          
             if (!guilds) return message.reply({ content: `:rolling_eyes: **صلاحيات ما عندي فگر**`, ephemeral: true }).catch((err) => {
                        console.log(`i couldn't reply to the message: ` + err.message)
      }) 
